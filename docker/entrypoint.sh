...
EOF
  ...
  # Execute any other command before we tail the log file and wait
  if [[ -n ${GENERAL_CMD} ]]; then
      sh -c "${GENERAL_CMD}"
  fi
  for n in {1..30}; do
    if [[ -n $(eval echo \$\{GENERAL_CMD_${n}\}) ]]; then
      sh -c "$(eval echo \$\{GENERAL_CMD_${n}\})"
    else
      # We do not want to iterate all, if one in the sequence is not set
      break
    fi
